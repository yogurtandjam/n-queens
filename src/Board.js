 ...
 */
    ...
    var row = 0;
    var sum = 0;
    for (var col = columnIndex; col < this.rows()[0].length; col++) {
      if(col>=0 && row <this.rows().length && this.get(row)[col]) {
    ...
    for (var col = columnIndex; col >= 0; col--) {
      if(col>=0 && row <this.rows().length && this.get(row)[col]) {
        sum += this.get(row)[col];
      }
      row++;
    ...
    for (var i = 2*this.rows()[0].length-2; i >=0 ; i--) {
